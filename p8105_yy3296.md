...
I set a function(n=30,mu=0,alpha=0.05,conf.level=0.95) for a one-sample
t-test.
...
The association between effect size and power:power will increase when
effect size increase.

I make a plot showing the average estimate of mu on the y axis and the
true value of mu on the x axis. Then, I make a second plot the average
...
<img src="p8105_yy3296_files/figure-gfm/unnamed-chunk-13-2.png" width="90%" />

By these two plots, we can see the first plot is almost a line, so we
can see the average estimated mean is approximately equal to the tru
mean; however, for the second plot, we can see the estimated mean is
higher than the true mean, so we get that the sample average of mu
across tests for which the null is rejected is not approximately equal
to the true value of mu. Since we need to make sample mean larger to
make the sample average of mu across tests for which the null is
